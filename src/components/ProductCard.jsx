    ...
    >
      ...
      {product.thumbnails?.length > 0 && (
        <div className="flex gap-1 px-3 pt-3">
          {[product.main_image_url, ...product.thumbnails].slice(0, 4).map((img, i) => (
            <button
              key={i}
      ...
      <div className="px-3 pt-3 pb-4">
        <h3 className="text-white font-display text-base font-semibold leading-tight mb-1 truncate">{product.name}</h3>
        <div className="flex items-center justify-between mb-4">
          <span className="text-white/90 font-sans font-semibold text-sm">₦{Number(product.price).toLocaleString()}</span>
        ...
        >
          {added ? <><Check className="w-3 h-3" /> Added</> : <><ShoppingBag className="w-3 h-3" /> Add to Cart</>}
        </motion.button>
      </div>
    </motion.div>
  )
